...
function desiredCount() {
  const areaCount = Math.round((width * height) / 23000);
  return width < 600 ? clamp(areaCount, 20, 32) : clamp(areaCount, 34, 72);
}
...
function drawLinks() {
  const maximumDistance = width < 600 ? 78 : 108;
  for (let first = 0; first < particles.length; first += 1) {
    for (let second = first + 1; second < particles.length; second += 1) {
      ...
      const opacity = (1 - distance / maximumDistance) * 0.13;
...
function drawParticles() {
  for (const particle of particles) {
    ...
    if (particle.marker) {
      context.strokeStyle = `rgba(${palette.accent.join(',')},0.32)`;
      context.lineWidth = 0.55;
      context.beginPath();
